...
@section('title', 'Destinasi - Wisata Sukabumi')

@section('content')
<div class="space-y-6 sm:space-y-8">
    
    <!-- Header Section -->
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6 bg-white p-5 sm:p-6 rounded-2xl sm:rounded-3xl border-2 border-slate-900 shadow-[4px_4px_0px_0px_#cc3366]">
        <div>
            <h1 class="text-xl sm:text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2.5 sm:gap-3">
                <span class="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-pink-600 rounded-full inline-block border border-slate-900"></span>
                Kelola Destinasi Wisata
            </h1>
            <p class="text-xs sm:text-sm text-slate-500 mt-1 sm:mt-1.5">Atur tautan destinasi wisata Sukabumi Anda dengan gaya modern interaktif.</p>
        </div>
        <!-- Perubahan elemen button menjadi tag anchor (a) -->
<a href="{{ route('admin.links.create') }}"
   class="w-full sm:w-auto bg-pink-400 hover:bg-pink-300 text-slate-950 font-extrabold py-2.5 sm:py-3 px-6 rounded-xl sm:rounded-2xl border-2 border-slate-900 shadow-[3px_3px_0px_0px_#cc3366] hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none transition-all flex items-center justify-center gap-2">
    ...
    <!-- Data List Container -->
    <div class="bg-rose-100 rounded-2xl sm:rounded-3xl border-2 border-slate-900 shadow-[4px_4px_0px_0px_#cc3366] sm:shadow-[6px_6px_0px_0px_#cc3366] overflow-hidden flex flex-col">

        <!-- Table Header (Desktop Only) -->
        <div class="hidden lg:grid grid-cols-12 gap-4 bg-rose-100 text-slate-900 px-6 py-4 border-b-2 border-slate-900 text-xs font-black uppercase tracking-wider">
            <div class="col-span-5">Destinasi & Lokasi</div>
        ...
        <div class="divide-y-2 divide-slate-900 bg-white">
            @forelse($links as $link)
                <div class="flex flex-col lg:grid lg:grid-cols-12 gap-4 lg:gap-4 items-start lg:items-center p-4 sm:p-6 lg:p-6 hover:bg-rose-50/60 transition-colors group">
                    ...
                    <div class="lg:col-span-5 flex items-center space-x-3 sm:space-x-4 w-full">
                        ...
                        @else
                            <div class="flex-shrink-0 h-10 w-10 sm:h-12 sm:w-12 bg-pink-200 text-slate-900 font-black border-2 border-slate-900 flex items-center justify-center rounded-lg sm:rounded-xl shadow-[2px_2px_0px_0px_#0f172a]">
                                {{ strtoupper(substr($link->title, 0, 2)) }}
                            </div>
                        @endif

                        <div class="overflow-hidden">
                            <div class="text-sm sm:text-base font-extrabold text-slate-900 group-hover:text-pink-800 transition-colors truncate">{{ $link->title }}</div>
                        ...
                        <div class="lg:col-span-2 flex flex-col lg:flex-row items-start lg:items-center flex-1">
                            ...
                            @if($link->is_active)
                                <span class="px-2.5 py-1 inline-flex text-xs font-extrabold rounded-md sm:rounded-lg bg-rose-200 text-rose-900 border-2 border-slate-900 shadow-[2px_2px_0px_0px_#0f172a] items-center gap-1.5 whitespace-nowrap">
                                    <span class="w-1.5 h-1.5 bg-yellow-300 rounded-full border border-slate-900"></span> Aktif
                                </span>
                            @else
                                <span class="px-2.5 py-1 inline-flex text-xs font-extrabold rounded-md sm:rounded-lg bg-rose-200 text-rose-900 border-2 border-slate-900 shadow-[2px_2px_0px_0px_#0f172a] items-center gap-1.5 whitespace-nowrap">
                                    <span class="w-1.5 h-1.5 bg-yellow-300 rounded-full border border-slate-900"></span> Non-Aktif
                                </span>
                            @endif
                        </div>
