...
<div class="max-w-3xl mx-auto space-y-6 sm:space-y-8">
    ...
    <!-- Header Section -->
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-pink-100 p-5 sm:p-6 rounded-2xl border-2 border-slate-900 shadow-[4px_4px_0px_0px_#cc3366]">
        <div>
            <h1 class="text-xl sm:text-2xl font-black text-slate-900 flex items-center gap-2.5">
                <a href="{{ route('admin.links.index') }}" class="bg-pink-200 hover:bg-pink-400 p-1.5 rounded-lg border-2 border-slate-900 shadow-[2px_2px_0px_0px_#cc3366] transition-all">
    ...
    <div class="bg-pink-100 rounded-2xl border-2 border-slate-900 shadow-[4px_4px_0px_0px_#cc3366] p-6 sm:p-8">
        ...
        <form action="{{ route('admin.links.update', $link->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <!-- Field: Judul Tautan -->
            <div class="space-y-2">
                <label for="title" class="block text-sm font-extrabold text-slate-900">Destinasi <span class="text-rose-500">*</span></label>
                <input type="text" id="title" name="title" value="{{ old('title', $link->title) }}" required
                       class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-900 rounded-xl focus:outline-none focus:ring-4 focus:ring-pink-950/20 font-medium">
                @error('title')
            ...
            <!-- Field: URL Tujuan -->
            <div class="space-y-2">
                <label for="url" class="block text-sm font-extrabold text-slate-900">Lokasi Wisata <span class="text-rose-500">*</span></label>
            ...
            <!-- Field: Deskripsi  -->
            <div class="space-y-2">
                <label for="description" class="block text-sm font-extrabold text-slate-900">Deskripsi <span class="text-rose-500">*</span></label>
                <textarea id="description" name="description" rows="4" placeholder="Deskripsi singkat tentang lokasi wisata" required
                          class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-900 rounded-xl focus:outline-none focus:ring-4 focus:ring-pink-950/20 font-medium">{{ old('description', $link->description) }}</textarea>
                @error('description')
            ...
            <div class="space-y-3">
                <label class="block text-sm font-extrabold text-slate-900">Foto Tempat <span class="text-slate-400 font-medium">(Opsional)</span></label>
                ...
                <div class="p-4 border-2 border-slate-200 border-dashed rounded-xl bg-slate-50">
                    ...
                    @if($link->image)
                        <img src="{{ asset('storage/' . $link->image) }}" class="h-16 w-16 object-cover border-2 border-slate-900 rounded-xl shadow-[2px_2px_0px_0px_#cc3366]" alt="Gambar Saat Ini">
                    @else
                        <span class="inline-block px-3 py-1.5 bg-pink-300 border-2 border-slate-300 rounded-lg text-xs font-bold text-slate-500">Belum Ada Gambar</span>
                    @endif
                </div>
                ...
                <div id="preview-wrapper" class="relative overflow-hidden rounded-2xl border-2 border-slate-900 bg-slate-50 transition-colors duration-200">
                    <div id="preview-empty" class="flex flex-col items-center justify-center gap-3 py-8 px-6 cursor-pointer hover:bg-slate-100 text-center">
                        <div class="w-12 h-12 rounded-2xl bg-pink-300 border-2 border-slate-900 flex items-center justify-center shadow-[3px_3px_0px_0px_#cc3366]">
                    ...
                    <div id="preview-filled" class="hidden">
                        ...
                        <div class="flex justify-between items-center p-4 bg-white border-t-2 border-slate-900">
                            ...
                            <button type="button" id="preview-remove" class="text-xs text-rose-700 bg-rose-100 font-extrabold px-3 py-1.5 rounded-lg border-2 border-slate-900 shadow-[2px_2px_0px_0px_#cc3366]">Batal Ganti</button>
                        </div>
                    </div>
                </div>
            ...
            <div class="pt-2">
                <label for="is_active" class="cursor-pointer select-none">
                    <div class="flex items-center justify-between gap-4 bg-slate-50 border-2 border-slate-900 rounded-2xl px-4 sm:px-5 py-3.5 shadow-[3px_3px_0px_0px_#cc3366] transition-all hover:shadow-[5px_5px_0px_0px_#cc3366]">
                        <div class="flex items-center gap-3">
                            <span class="bg-pink-100 text-pink-800 p-2 rounded-xl border border-pink-200">
                        ...
                        <span class="relative w-12 h-7 bg-slate-300 peer-checked:bg-pink-300 rounded-full border-2 border-slate-900 transition-colors duration-300 shrink-0 after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:w-5 after:h-5 after:bg-white after:rounded-full after:border-2 after:border-slate-900 after:transition-transform peer-checked:after:translate-x-5"></span>
                    </div>
                </label>
            </div>
            ...
            <div class="pt-6 flex justify-end gap-3 border-t-2 border-dashed border-slate-200">
                <a href="{{ route('admin.links.index') }}" class="bg-pink-100 text-slate-900 font-extrabold py-3 px-6 rounded-xl border-2 border-slate-900 shadow-[3px_3px_0px_0px_#cc3366]">Batal</a>
                <button type="submit" class="bg-pink-400 hover:bg-pink-200 text-slate-950 font-extrabold py-3 px-8 rounded-xl border-2 border-slate-900 shadow-[3px_3px_0px_0px_#cc3366] flex items-center gap-2">
